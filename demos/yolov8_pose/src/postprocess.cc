...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "./model/pose_labels_list.txt"
...
static int process_i8_rv1106_pose(int8_t *box_tensor, int32_t box_zp, float box_scale,
                             int8_t *score_tensor, int32_t score_zp, float score_scale,
                             int8_t *score_sum_tensor, int32_t score_sum_zp, float score_sum_scale,
                             int8_t *pose_tensor, int32_t pose_zp, float pose_scale,
                             int grid_h, int grid_w, int stride, int dfl_len, int n_kpts,
                             std::vector<float> &boxes,
                             std::vector<float> &objProbs,
    ...
    for (int i = 0; i < grid_h; i++) {
        for (int j = 0; j < grid_w; j++) {
            ...
            if (max_score > score_thres_i8) {
                ...
                offset = (i * grid_w + j) * 3 * n_kpts;
                
                for (int k = 0; k < n_kpts; k++) {
                    float kpt_x = (deqnt_affine_to_f32(pose_tensor[offset + k * 3 + 0], pose_zp, pose_scale) * 2.0 + j) * stride;
                    float kpt_y = (deqnt_affine_to_f32(pose_tensor[offset + k * 3 + 1], pose_zp, pose_scale) * 2.0 + i) * stride;
                    float kpt_conf = sigmoid(deqnt_affine_to_f32(pose_tensor[offset + k * 3 + 2], pose_zp, pose_scale));
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    // default 3 branch
    int dfl_len = app_ctx->output_attrs[0].dims[3] / 4; // NHWC
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (app_ctx->is_quant) {
            ...
            validCount += process_i8_rv1106_pose((int8_t *)_outputs[box_idx]->virt_addr, app_ctx->output_attrs[box_idx].zp, app_ctx->output_attrs[box_idx].scale,
                                (int8_t *)_outputs[score_idx]->virt_addr, app_ctx->output_attrs[score_idx].zp, app_ctx->output_attrs[score_idx].scale, 
                                (int8_t *)score_sum, score_sum_zp, score_sum_scale,
                                (int8_t *)_outputs[pose_idx]->virt_addr, app_ctx->output_attrs[pose_idx].zp, app_ctx->output_attrs[pose_idx].scale,
                                grid_h, grid_w, stride, dfl_len, n_kpts, filterBoxes, objProbs, kpts, classId, conf_threshold);
        }
        else
        {
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        {
            float kpt_x = kpts[n * 3 * n_kpts + j * 3 + 0] - letter_box->x_pad;
            float kpt_y = kpts[n * 3 * n_kpts + j * 3 + 1] - letter_box->y_pad;
            float kpt_conf = kpts[n * 3 * n_kpts + j * 3 + 2];
            od_results->results[last_count].kpts.push_back(
                std::pair<int, int>(
                    (int)(clamp(kpt_x, 0, model_in_w) / letter_box->scale), 
                    (int)(clamp(kpt_y, 0, model_in_h) / letter_box->scale)));
            od_results->results[last_count].kpts_scores.push_back(kpt_conf);
        }
