...
int init_arcface_model(const char *model_path, rknn_app_context_t *app_ctx)
{
    int ret;
    int model_len = 0;
    char *model;
...
int release_arcface_model(rknn_app_context_t *app_ctx)
{
...
int inference_arcface_model(rknn_app_context_t *app_ctx, image_buffer_t *img, face_rec_result *od_results)
{
    int ret;
    ...
    int8_t *output_tensor = (int8_t *)app_ctx->output_mems[0]->virt_addr;
    float output_zp = app_ctx->output_attrs[0].zp;
    float output_scale = app_ctx->output_attrs[0].scale;

    for (int i = 0; i < 512; i++) {
        // printf("output_tensor[%d]=%d\n", i, output_tensor[i]);  
        od_results->embedding[i] = deqnt_affine_to_f32(output_tensor[i], output_zp, output_scale);
    }
